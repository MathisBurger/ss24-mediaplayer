...
public class ComparatorTest {
    ...
    public void testRetValsOfComparators() {
        ...
        Assert.assertTrue("Sortierung nach Interpret falsch", ic.compare(mp3, wav) < 0);
        
        // lexikographisch: wellenmeister > Eisbach
        Assert.assertTrue("Sortierung nach Interpret falsch", ic.compare(wav, mp3) > 0);

        TitleComparator tc = new TitleComparator();
        // lexikographisch: Rock < tranquility
        Assert.assertTrue("Sortierung nach Titel falsch", tc.compare(mp3, wav) < 0);
        
        // lexikographisch: tranquility > Rock
        Assert.assertTrue("Sortierung nach Titel falsch", tc.compare(wav, mp3) > 0);

        DurationComparator dc = new DurationComparator();
        // 05:31 > 02:21 , entsprechendes gilt für die Dauer in Mikrosekunden
        Assert.assertTrue("Sortierung nach Dauer falsch", dc.compare(mp3, wav) > 0);

        // 02:21 < 05:31 , entsprechendes gilt für die Dauer in Mikrosekunden
        Assert.assertTrue("Sortierung nach Dauer falsch", dc.compare(wav, mp3) < 0);
